...
function Level01()
 return
	  Player(L(1))
		,Player(R(1))
 	,{Ball(2,1,L(4),B(4), PSPEED,0)
			,Ball(1,1,R(4),B(4),-PSPEED,0)
	  }
end
...
function Level02()
	return
	  Player(L(1))
		,Player(R(1))
 	,{Ball(1,1,L(4),B(4), PSPEED,0)
			,Ball(2,1,R(4),B(4),-PSPEED,0)
	  }
end
...
function Level03()
 return
	  Player(L(3))
		,Player(R(3))
 	,{Ball(2,2,L(4),B(4), PSPEED,0)
			,Ball(1,2,R(4),B(4),-PSPEED,0)
	  }
end
...
function Level05()
 return
	  Player(L(3))
		,Player(R(3))
 	,{Ball(2,4,L(4),B(10), PSPEED,0)
			,Ball(1,4,R(4),B(10),-PSPEED,0)
	  }
end
...
function RectRect(a,b)
 return
 	a.pos[1]<b.pos[1]+b.w and
  a.pos[2]<b.pos[2]+b.h and
  b.pos[1]<a.pos[1]+a.w and
  b.pos[2]<a.pos[2]+a.h
end
 ...
 p2.safe=p2.safe-dt*1000
   ...
   p.hook={p.pos,0}
	...
	function IntersectHook(p,pt)
	  ...
	  local hook=
	   {pos={p.hook[1]-2,bounds[3]-p.hook[2]}
	   ,w=4
	   ,h=p.hook[2]
	   }
		 for i,ball in pairs(balls) do
